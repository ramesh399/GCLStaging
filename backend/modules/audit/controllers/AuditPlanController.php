...
class AuditPlanController extends \yii\rest\Controller
{
	...
	public function actionCreateAuditPlan()
    {
		...
		$responsedata=array('status'=>0,'message'=>'Something went wrong! Please try again');
        $data = Yii::$app->request->post();
		if( (count($data['units'])>1) && (!$this->checkDiffAud($data))){
			return $responsedata=array('status'=>0,'message'=>'Please select different dates for different units!');
		}
		...
		if ($data) 
		{
			...
			{ 
				...
				$unitssectorgroups = [];
				$date_id = isset($data['date_id'])?$data['date_id']:'';
				$type = isset($data['type'])?$data['type']:'';

				if($this->checkDate($dateformatted,$auditorids,$date_id,$type)){
					
					return $responsedata=array('status'=>0,'message'=>'Auditor is not available!');
				}
		...
		if ($data) 
		{
			...
			*/
			...
			if ($modelModel !== null)
			{
				...
				$resultarr["sel_brand_ch"]=$modelModel->application->is_brand;
				...
				*/
				...
				$mandayarray = ApplicationUnitManday::find()->where(['app_id'=>$modelModel->app_id])->all();
				$mandayarr=array();
                if (count($mandayarray)>0) {
                    foreach ($mandayarray as $manday) {
						...
						$mandayssarr['actual_manday']=isset($manday->audiplanhistory->actual_manday)?$manday->audiplanhistory->actual_manday:"";
	...
	private function checkDiffAud($data){
		// echo json_encode($data);
		$count=0;
		foreach($data['units'] as $val){
			if(is_array($val['auditor']) && count($val['auditor'])>0){
				foreach($val['auditor'] as $aud){
					if(!$this->checkAudi($aud['user_id'],$aud['date'],$data,$count)){
						return false;
					}
				}
			}	
	...
	private function checkAudi($id,$date,$unidata,$count){
		
			

				foreach($unidata['units'] as $uniKey=>$unival){
					if($uniKey!=$count){
					// echo json_encode($unival);
					if(is_array($unival['auditor']) && count($unival['auditor'])>0){
						foreach($unival['auditor'] as $temaud){
							if($temaud['user_id']==$id){
								if(array_intersect($date,$temaud['date'])!=='' && array_intersect($date,$temaud['date'])!=null){
									// echo json_encode($temaud['user_id']);
									// echo $count;
	...
	private function checkDate($auditorDates,$auditorIDs,$date_id,$type){
		// echo 'insied';
		$connection = Yii::$app->getDb();						
		$connection->createCommand("SET SQL_BIG_SELECTS = 1")->execute();
		$currentdate = date('Y-m-d');
		//echo strtotime($currentdate);
		foreach($auditorIDs as $ids){
			
			$audmodeldates = AuditPlanUnitAuditor::find()->alias('aud');
			$audmodeldates = $audmodeldates->join('inner join','tbl_audit_plan_unit_auditor_date as aud_date','aud.id=aud_date.audit_plan_unit_auditor_id');
			$audmodeldates = $audmodeldates->where(['aud.user_id'=>$ids]);
			$audmodeldates = $audmodeldates->andWhere(['>=','aud_date.date',date('Y-m-d')]);

			if($type==2){
				// echo 'inside inside';
				$audmodeldates = $audmodeldates->andWhere(['aud_date.audit_plan_unit_auditor_id'=>!$date_id]);
			}

			$audmodeldates = $audmodeldates->all();

			if(count($audmodeldates)>0){
				// echo 'inside inside inside';
				foreach($audmodeldates as $auddates){
					// echo $auddates->id;
					$datemodel = AuditPlanUnitAuditorDate::find()->where(['audit_plan_unit_auditor_id'=>$auddates->id])->all();
					if(count($datemodel)>0){
						foreach($auditorDates as $auddatesdate){
							foreach($datemodel as $adates){
								if(strtotime($auddatesdate) == strtotime($adates['date'])){	
									return true;
								}
							}
						}
			...
			if ($model !== null)
			{
				...
				if(isset($data['status'])){
					if($data['status'] == $auditmodel->arrEnumStatus['review_in_process']){
						...
						if($auditmodel->audittypeEnumArr['unannounced_audit'] == $model->audit_type){
							$canDoAction = 1;
						}
						if($canDoAction==0){ return $responsedata; }
	...
	*/
    ...
    public function actionListAuditPlan()
    {
		...
		$model = $model->andWhere(['!=','tbl_audit.status', $modelAudit->arrEnumStatus['finalized_without_audit'] ]);
		...
		*/
		...
		if(count($model)>0)
		{
			foreach($model as $audit)
			{
				...
				$data['customer_number']=$audit->application && $audit->application->customer?$audit->application->customer->customer_number:'';	
					...
					if($audit_type  != 2){
						...
						if($checkenvironment && $formstatus){
							...
							if(count($modelAuditReportEnvironment)<=0)
							{
								...
								$modelAuditReportEnvironment = AuditReportEnvironment::find()->where(['app_id' => $appID,'unit_id' => $unitID,'sufficient'=>null])->one();
								if($modelAuditReportEnvironment!==null){
									$innerContent.='<li>Environment is Sufficient for '.$appunit->name.'.</li>';	
									$reportFillStatus=false;
								}
							}
	...
	private function canViewAuditPlan($audit_id){
		...
		if($Audit !== null){
			...
			if($Audit->status == $Audit->arrEnumStatus['approved'] || $Audit->status > $Audit->arrEnumStatus['rejected']){
				...
				$auditorList = [];
				if($auditplanunit != null && count($auditplanunit)>0){
