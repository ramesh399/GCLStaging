...
class UserDashboard extends Model
{
    ...
    */
    ...
    public function brandWaitingForApproval($franchiseid,$userid)
	{
		$application = new Application();
		$resultarr=array();
		$brdmodel = Brand::find()->where(['user_id'=>$userid])->one();
		...
		if($brdmodel!='' && $brdmodel!==null){
			...
			if(count($appbrandmod)>0){
				...
				foreach($appbrandmod as $app){
					$data=array();
					$data['app_id'] =$app['app_id'];
					$data['id']=$app['id'];
					$data['brand_name']=$brdmodel->name;
					$data['brand_id']=$brdmodel->id;
