...
import { FollowupAuditPlanService } from '@app/services/audit/followup-audit-plan.service';
...
@Component({
  selector: 'app-followup-audit-plan',
  templateUrl: './followup-audit-plan.component.html',
  styleUrls: ['./followup-audit-plan.component.scss'],
  encapsulation: ViewEncapsulation.None
})
export class FollowupAuditPlanComponent implements OnInit {

	title = 'Followup Audit Planning';
	form : any;
	loading = false;
	buttonDisable = false;
	error:any='';
	...
	constructor(private activatedRoute:ActivatedRoute,private router: Router,private fb:FormBuilder,private auditPlanService: FollowupAuditPlanService,private errorSummary: ErrorSummaryService) { }
	...
	ngOnInit() {
		...
		this.audit_type = this.activatedRoute.snapshot.queryParams.audit_type;
		this.month[0] = "Jan";
		this.month[1] = "Feb";
		this.month[2] = "Mar";
		this.month[3] = "Apr";
		...
		if(this.audit_type == 2){
			this.title = 'Unannounced Audit Plan';
		}
	...
	*/
		this.auditPlanService.getAuditPlan({audit_type:this.audit_type,audit_id:this.id,app_id:this.app_id,invoice_id:this.invoice_id,offer_id:this.offer_id}).pipe(first())
    	.subscribe(res => {
			this.auditPlanData = res;	  
			let appLeadAuditors = [];

			this.share_plan_to_customer = res.share_plan_to_customer;
			this.unannounced_audit_reason = res.unannounced_audit_reason;
			if(res['units'] !== undefined && res['units'].length > 0 ){
				...
				if(this.audit_type == 2 && res['followup_status'] == '1'){
					this.application_lead_auditor = res['followup_application_lead_auditor'];
		...
		*/
		...
		this.auditPlanService.getAuditPlanLoadDetails({audit_id:this.id}).pipe(first())
    	.subscribe(res => {
			this.sharePlanArr = res['sharePlanArr'];
		});
	}
	...
	getAuditors(index,unitid){
		...
		*/
		...
		if(unit_auditor_date!== undefined && unit_auditor_date !=''){
			...
			this.auditPlanService.getAuditors({audit_id:this.id,sector_group_ids:sector_group_ids,unitstandards:unitstandards,app_id:this.app_id,unitid:unitid,dates:this.auditplanlist['unit_date_'+unitid]})
				...
				if(res.status){
					...
					if(addedusers && addedusers.selauditors && addedusers.selauditors.length>0){
						addedusers.selauditors.forEach(auditorid=>{
							...
							//business_sector_groups_id
							//business_sector_groups.foreach()
	...
	*/
	...
	setUnitAuditor(unitindex,unitid)
	{
		...
		*/
		...
		//this.f.auditor_name.setValidators([Validators.required]);
		//this.f.auditor_dates.setValidators([Validators.required,Validators.pattern(/^\d*\.?\d{0,2}$/g),Validators.max(100)]);

		//this.f.auditor_name.updateValueAndValidity();
		//this.f.auditor_dates.updateValueAndValidity();

		//this.touchUnitAuditor();
	}
	...
	addUnitAuditor(unitindex,unitid){
		...
		//console.log('--'+this.auditorlist['lead_auditor_'+unitid]);
		//console.log(this.auditplanlist['unit_date_'+unitid]+'---'+this.auditplanlist['lead_auditor_'+unitid]);
	...
	calculateManday(ut,unitid)
	{
		...
		if(this.appAuditorsEntries && this.appAuditorsEntries[ut] && this.appAuditorsEntries[ut].length>0){
  ...
  //get f() { return this.NgForm.controls; }
	...
	*/
	...
	this.auditPlanData.units.forEach((value,key,myMap)=> {	
		...
		//console.log(this.appActualMandayData[key]);
		if(this.appActualMandayData[key] =='' || this.appActualMandayData[key]===undefined || value.quotation_manday>this.appActualMandayData[key]){
			unitMandayErr.push(value.name);
		}

        let unitData = {unit_id:unitid,unit_lead_auditor:this.auditplanlist['lead_auditor_'+unitid],technical_expert:this.auditplanlist['technical_expert_'+unitid],translator:this.auditplanlist['translator_'+unitid],standard:unitStd,date:unitSelectedD,auditor:unitAuditorWithDate,quotation_manday:value.quotation_manday,actual_manday:this.appActualMandayData[key],observer:this.auditplanlist['observer_'+unitid],trainee_auditor:this.auditplanlist['trainee_auditor'+unitid]};
		unit_data.push(unitData);
    });
	//return false;
	let app_data:any = {sectorwiseusersList:this.sectorwiseusersList,audit_id:this.id,app_id:this.app_id,offer_id:this.offer_id,invoice_id:this.invoice_id,units:unit_data,application_lead_auditor:f.value.application_lead_auditor};
	if(this.audit_type == 2 && this.auditPlanData.followup_status != 1){
		if(f.value.share_plan_to_customer === '' || f.value.share_plan_to_customer === undefined){
			errMsg.push('<li>Please select Share Audit Plan to Customer</li>');
			formerror = true;
		}
		if(f.value.unannounced_audit_reason == '' || f.value.unannounced_audit_reason === undefined || f.value.unannounced_audit_reason.trim() == ''){
			errMsg.push('<li>Please etner the Reason</li>');
			formerror = true;
		}
		app_data.unannounced_audit_reason = f.value.unannounced_audit_reason;
		app_data.share_plan_to_customer = f.value.share_plan_to_customer;
	...
	*/
	...
	if (!formerror) {
		...
		this.loading = true;
		if(this.audit_type == '1' || (this.audit_type == '2' && this.auditPlanData.followup_status == '1')){
			console.log('333');
			...
			this.auditPlanService.createNormalAuditPlan(app_data).pipe(first()).subscribe(res => {
