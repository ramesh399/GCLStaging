...
export class EditUserComponent implements OnInit {
  ...
  let  bussinessSector = this.auditExpForm.get('business_sector').value;
    ...
    if(this.auditExpForm.valid)
    {
      ...
      audexperiencedatas.push({id:id,standard:standard,businesssector:bussinessSector,year:year,company:company,cb:cbval,days:days});
  ...
  onStandardSubmit()
  {
    ...
    if(stdid){
      ...
      if(selStd.code == 'GRS' || selStd.code == 'GOTS'){
        ...
        }else if(this.qua_examfileErrors==''){
  ...
  onAudExperienceSubmit(){
    if (this.userData.audit_experience.length<=0 && this.auditexperienceEntries.length<=0) {
      ...
      this.auditexperienceEntries.forEach((val)=>{
        //,process:val.process
        audexperiencedatas.push({standard:val.standard,year:val.year,company:val.company,cb:val.cb,days:val.days})
      });
